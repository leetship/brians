...
const getFiles = (path) => {
    ...
    fs.readdirSync(path)
        .sort()
        .forEach((file) => {
            if (file.startsWith(".") || file == "CrossCritter") {
                return;
            }
            files.push(file);
        });
...
exports.mockDeploymentData = (path) => {
    const metadata = getImagesWithMetadata(path);
    const typesOptions = [...new Set(metadata.map((i) => i.type))];
    const backgroundsOptions = [...new Set(metadata.map((i) => i.background))];

    return {
        backgroundsOptions: backgroundsOptions,
        typesOptions: typesOptions,
        traitsVarieties: metadata.map((i) => i.variety),
        traitsTypes: metadata.map((i) => typesOptions.indexOf(i.type)),
        traitsColors: metadata.map((i) =>
            backgroundsOptions.indexOf(i.background)
        ),
        traitsImages: metadata.map((i) => i.image),
    };
};

// encodePic(path.join(__dirname, "../assets/p1x/5p4c3c47.svg"));
exports.encodePic = (path) => {
    let colors = [];
...
exports.getImagesWithEqualRarities = (path) => {
    let names = [];
    let images = [];
    let rarities = [];
    ...
    for (let i = 0; i < dirs.length; i++) {
        ...
        names.push(dirs[i].split(".png")[0]);
        images.push(data);
    }

    const size = names.length;
    const rarity = Math.floor(10000 / size);
    for (let i = 0; i < size; i++) {
        rarities.push(rarity);
    }
    const sum = rarities.reduce((a, b) => a + b);
    rarities[size - 1] = rarities[size - 1] + (10000 - sum);
    return {
        names: names,
        images: images,
        rarities: rarities,
