...
contract LeetBrian is ERC721A, ERC721AQueryable, Ownable {
    ...
    constructor(string memory name, string memory symbol, uint256 totalSupply, bytes32 raritiesHash)
        ERC721A(name, symbol)
    {
        supply = totalSupply;
    ...
    function checkWhitelist(address addr, bytes32[] calldata merkleProof) public view returns (bool) {
        return MerkleProof.verifyCalldata(merkleProof, _merkleRoot, keccak256(abi.encodePacked(addr)));
    }
     ...
     * @notice Add traits
     */
    function addTraits(uint8 layer, Payload[] calldata payload) public onlyOwner {
     ...
     */
    function whitelistMint(bytes32[] calldata merkleProof, uint256[][] calldata rarities) public {
     ...
     * @notice Owner mint
     */
    function ownerMint(uint256 amount, uint256[][] calldata rarities) public onlyOwner {
     ...
     */
    function airdrop(uint256 amount, uint256[][] calldata rarities, address to) public onlyOwner {
     ...
     */
    function _mint(uint256 amount, uint256[][] calldata rarities, address to) private {
     ...
     */
    function _getTraitImage(Brian memory brian) internal view returns (string memory image) {
     ...
     */
    function _getTraitImageData(address image) private view returns (string memory) {
        return string(abi.encodePacked("url(data:image/png;base64,", Base64.encode(SSTORE2.read(image)), ")"));
    }

    /**
     * @notice Get trait attributes
     */
    function _getTraitAttributes(Brian memory brian) internal view returns (string memory trait) {
        return string.concat(
            _getTraitMetadata("background", _traits[0][brian.background].name),
            ",",
     ...
     */
    function _getTraitMetadata(string memory key, string memory value) internal pure returns (string memory trait) {
     ...
     */
    function _setTraitsCombination(uint256 tokenID, uint256 amount, uint256[][] calldata rarities) private {
     ...
     */
    function _getRandomTraitIndex(uint256[] calldata rarities, uint256 seed) private pure returns (uint256 index) {
     ...
     */
    function _getTraits(uint256 tokenId) internal view returns (Brian memory brian) {
        uint256 tokenTraitBucket = tokenId / 4;
        uint256 tokenTraitSlot = tokenId % 4;
